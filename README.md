...
- **装完 `claude: command not found`？** 新开一个终端再试（PATH 已写入 rc）；或临时 `export PATH="$HOME/.local/bin:$PATH"`。
- **"配了但不生效 / 还在连官方 Anthropic"？** 多半是 shell 里有旧 `ANTHROPIC_*` 环境变量盖过了 settings.json——脚本会探测并（经确认后）注释掉它们；若仍在，查 `~/.zshrc`/`~/.bashrc` 及项目根的 `./.claude/settings.json`。
- **不用 `/login`。** 这是中转模式：已配好 `ANTHROPIC_AUTH_TOKEN` 指向 apiget，**无需登录 Anthropic 账号**；进对话后 `/status` 可确认中转已生效。
- **国内装不上 / `claude.ai` 拉不动？** 脚本自动回退国内镜像 npmmirror；也可显式 `curl -fsSL https://get.apiget.cc | bash -s -- --cn`。
- **第一句很慢 / 偶发超时？** 已默认 `API_TIMEOUT_MS=600000`（中转链路较长时给足时间），网络抖动重试即可。
- **想重来 / 撤销？** `... | bash -s -- --uninstall` 还原配置（不卸载 Claude Code 本身）。
- **Windows 要 WSL 吗？** 不需要。`install.ps1` 原生 PowerShell（自动放行当前进程执行策略）；想让 CC 的 Bash 工具更顺可选装 Git for Windows（不装则用 PowerShell）。
